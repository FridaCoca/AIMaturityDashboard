...
def calculate_Punkte():
    cols_to_sum = ['MA1','MA2','MA3','MA4','MA5']
...
df.columns = ['index', 'Frage', 'MA1', 'MA2','MA3', 'MA4', 'MA5']
assign_dimension()
df.drop([0,1], axis=0,inplace=True)
df.drop([23,48,59,63], axis=0,inplace=True)
calculate_Punkte()
st.write(df)
...
dimension = st.sidebar.multiselect(
    "Gestaltungsdimension auswählen:",
    options=df[strIdentifingColumn].unique(),
    # default=df[strIdentifingColumn].unique
)
dfSelection = df.query(
    "Gestaltungsdimension == @dimension"
)
